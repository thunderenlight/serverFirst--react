...
// @desc Change  current todo to complete
// @Private
...
router.put("/:toDoId/complete", requiresAuth, async(req,res) =>{
    try{
        ...
        if(toDo.complete) {
            return res.status(500).json({ error: "ToDo is already marked as complete" });
        };
...
// @desc Change  current todo to incomplete
// @Private
